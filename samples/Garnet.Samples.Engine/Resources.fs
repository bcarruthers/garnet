...
type ReadOnlyFolderCollection() =
    let sources = List<IReadOnlyFolder>()
...
type ResourceSet() =
    ...
    let sources = ReadOnlyFolderCollection()
    let invalidate = 
        Action<_>(fun key ->
    ...
    member c.InvalidateChanged() =
        sources.InvalidateChanged(invalidate)
    member c.Dispose() =
